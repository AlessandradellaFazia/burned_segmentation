...
def convert_swin(ckpt):
    ...
    inputs = new_ckpt["backbone.patch_embed.proj.weight"]
    logger.info(
        f"Original backbone.patch_embed.proj.weight shape: {inputs.shape}"
    )  # [96, 3, 4, 4]
    indices = [
        0,
        1,
    ...
    new_ckpt["backbone.patch_embed.proj.weight"] = inputs[:, indices, :, :]
    logger.info(
        f"New backbone.patch_embed.proj.weight shape: {new_ckpt['backbone.patch_embed.proj.weight'].shape}"
    )
